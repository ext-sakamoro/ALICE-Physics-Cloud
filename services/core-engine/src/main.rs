...
struct CollisionRequest {
    ...
    body_a: String,
    body_b: String,
    velocity_a: Option<[f64; 3]>,
    velocity_b: Option<[f64; 3]>,
}
#[derive(Serialize)]
...
struct ConstraintRequest {
    ...
    anchor_a: Option<[f64; 3]>,
    anchor_b: Option<[f64; 3]>,
}
#[derive(Serialize)]
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "physics_engine=info".into()),
        )
        .init();
...
async fn health(State(s): State<Arc<AppState>>) -> Json<Health> {
    ...
    Json(Health {
        ...
        total_ops: st.total_simulations + st.total_collisions + st.total_raycasts,
    })
}
...
async fn simulate(
    ...
    let energy = req.bodies as f64 * 9.81 * 10.0 + (h % 1000) as f64;
    {
        ...
        st.bodies_processed += req.bodies as u64;
    }
    Json(SimulateResponse {
        ...
        avg_penetration_mm: penetration,
...
async fn collision(
    State(s): State<Arc<AppState>>,
    Json(req): Json<CollisionRequest>,
    ...
    let depth = if detected {
        (h % 50) as f64 * 0.01
    } else {
        0.0
    };
    let impulse = if detected {
        (h % 200) as f64 + 10.0
    } else {
        0.0
    };
    ...
    Json(CollisionResponse {
        collision_id: uuid::Uuid::new_v4().to_string(),
        detected,
        contact_point: point,
        normal,
...
async fn raycast(
    ...
    let hit_pt = [
        req.origin[0] + req.direction[0] * dist,
        req.origin[1] + req.direction[1] * dist,
        req.origin[2] + req.direction[2] * dist,
    ];
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    ...
    Json(StatsResponse {
        ...
        total_raycasts: st.total_raycasts,
        bodies_processed: st.bodies_processed,
    })
}
